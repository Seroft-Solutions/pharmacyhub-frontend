 ...
 * It standardizes the representation of answers and provides conversion utilities.
 */

import { Question } from '../types/models/exam';
 ...
 */
export function findCorrectAnswerLabel(question: Question): AnswerLabel | null {
  // Case 1: Question already has a correctAnswer as a letter
  if (typeof question.correctAnswers?.[0] === 'string' && /^[A-Da-d]$/.test(question.correctAnswers[0])) {
    return question.correctAnswers[0].toUpperCase() as AnswerLabel;
  }
  
  // Case 2: One of the options has isCorrect=true
...
export function getUserSelectedLabel(userAnswer: any): AnswerLabel | null {
  ...
  if (userAnswer.selectedOptions && userAnswer.selectedOptions.length > 0) {
    return normalizeToLabel(userAnswer.selectedOptions[0]);
  }
