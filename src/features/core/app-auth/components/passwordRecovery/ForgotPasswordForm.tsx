...
import { AUTH_ENDPOINTS } from '@/features/core/app-auth/api/constants';
  ...
  const handleRequestReset = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        ...
        console.error("Hook method failed, checking error type", hookError);
        
        // Check if it's a timeout error
        const errorMessage = hookError instanceof Error ? hookError.message.toLowerCase() : String(hookError).toLowerCase();
        if (errorMessage.includes('timeout') || errorMessage.includes('408')) {
          console.log('Timeout error detected, showing success anyway', errorMessage);
          // For timeout errors, still show success since the backend might be processing
          setCurrentStep('success');
          return;
        }
        
        // For other errors, try a fallback direct API call
      ...
      // Check if it's a timeout error
      const errorMessage = fullError.toLowerCase();
      if (errorMessage.includes('timeout') || errorMessage.includes('408')) {
        console.log('Timeout error detected in catch block, showing success anyway');
        // For timeout errors, still show success since the backend might be processing
        setCurrentStep('success');
        return;
      }
